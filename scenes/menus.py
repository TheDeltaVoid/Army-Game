...
class MainMenu:
    ...
    def render(self):
        ...
        text_size = pyray.measure_text_ex(main_font, text.get_text("TITLE", self.current_player_prefs), TITLE_FONT_SIZE, TITLE_SPACING)
        text_pos = pyray.Vector2((pyray.get_render_width() // 2) - (text_size.x // 2), (text_size.y // 2))
        pyray.draw_text_ex(main_font, text.get_text("TITLE", self.current_player_prefs), text_pos, TITLE_FONT_SIZE, TITLE_SPACING, TITLE_COLOR)


        button_image = pyray.load_image(PLAY_BUTTON_IMAGE)

        button_text_size = pyray.measure_text_ex(main_font, text.get_text("PLAY", self.current_player_prefs), PLAY_BUTTON_TEXT_FONT_SIZE, PLAY_BUTTON_TEXT_SPACING)
        ...
        pyray.image_draw_text_ex(button_image, main_font, text.get_text("PLAY", self.current_player_prefs), button_text_pos, PLAY_BUTTON_TEXT_FONT_SIZE, PLAY_BUTTON_TEXT_SPACING, PLAY_BUTTON_TEXT_COLOR)
